...
function findBestCut(customH, customW, depth) {
...
export default function FilterCutDB() {
  ...
  const handleSearch = useCallback(() => {
    ...
    const r = findBestCut(h, w, depth);
    setResults(r);
    setSearched(true);
    setSelectedIdx(0);
  }, [customH, customW, depth]);
                    ...
                    <div className={`px-5 py-3 flex items-center justify-between ${selectedIdx===i?"bg-blue-100/50":i===0?"bg-emerald-100/50":"bg-slate-50"}`}>
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-slate-700">
                          Option {i+1} - {r.type==="single"?"Single Cut":r.layout==="grid"?`${r.gridRows}x${r.gridCols} Grid`:`${r.stockFilters.length}-Filter Butt`}
                        </span>
                      </div>
                      ...
                      <div className="px-5 py-3.5 bg-slate-50 flex items-center justify-between border-b border-slate-100">
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-xs text-slate-500">
                            {r.type==="single"?"Single Cut":r.layout==="grid"?`${r.gridRows}x${r.gridCols} Grid`:`${r.stockFilters.length}-Filter Butt`}
                          </span>
                        </div>
                        <div className="flex items-center gap-4">
                      ...
                      {cartItems.map((item, idx) => {
                        ...
                        return (
                          <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50">
                            ...
                            <td className="px-5 py-4 text-slate-500 text-xs">{r.type==="single"?"Single Cut":r.layout==="grid"?`${r.gridRows}x${r.gridCols} Grid`:`${r.stockFilters.length}-Filter Butt`}</td>
