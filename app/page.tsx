...
function findAllCuts(customH, customW, depth, qty) {
  const stocks = STOCK[depth]; if (!stocks) return null;
  const all = getStockOrientations(stocks);
  const isSafe = (t) => t === 0 || t >= 1;
  const results = [];

  // ── 1. MULTI-YIELD: 1 stock filter → 2 custom filters (only when qty > 1) ──
  // Only 2x1 and 1x2 configurations. Waste comes from the middle (cap/waste strip).
  if (qty > 1) {
    for (const f of all) {
      // 2x1: two filters stacked along height
      {
        const neededH = 2 * customH;
        if (f.actH >= neededH && f.actW >= customW) {
          const trimH = +(f.actH - neededH).toFixed(4);
          const trimW = +(f.actW - customW).toFixed(4);
          if (isSafe(trimH) && isSafe(trimW)) {
            const wasteArea = +((f.actH * f.actW) - (neededH * customW)).toFixed(4);
            const cuts = 1 + (trimH > 0 ? 1 : 0) + (trimW > 0 ? 1 : 0);
            results.push({
              type: "multi-yield-2x1", layout: "multi-yield", yieldAxis: "H",
              yieldsPerStock: 2, gridRows: 2, gridCols: 1,
              stockFilters: [{ ...f, origNomH: f.origNomH || f.nomH, origNomW: f.origNomW || f.nomW }],
              trimH, trimW, wasteArea, cuts,
              customActH: customH, customActW: customW, depth
            });
          }
        }
      }
      // 1x2: two filters side by side along width
      {
        const neededW = 2 * customW;
        if (f.actH >= customH && f.actW >= neededW) {
          const trimH = +(f.actH - customH).toFixed(4);
          const trimW = +(f.actW - neededW).toFixed(4);
          if (isSafe(trimH) && isSafe(trimW)) {
            const wasteArea = +((f.actH * f.actW) - (customH * neededW)).toFixed(4);
            const cuts = 1 + (trimH > 0 ? 1 : 0) + (trimW > 0 ? 1 : 0);
            results.push({
              type: "multi-yield-1x2", layout: "multi-yield", yieldAxis: "W",
              yieldsPerStock: 2, gridRows: 1, gridCols: 2,
              stockFilters: [{ ...f, origNomH: f.origNomH || f.nomH, origNomW: f.origNomW || f.nomW }],
              trimH, trimW, wasteArea, cuts,
              customActH: customH, customActW: customW, depth
            });
  ...
  // ── 2. SINGLE CUT: 1 stock filter → 1 custom filter ────────────────────────
  for (const f of all) {
    if (f.actH >= customH && f.actW >= customW) {
      ...
      results.push({ type:"single", stockFilters:[{ nomH:f.nomH, nomW:f.nomW, actH:f.actH, actW:f.actW, rotated:f.rotated, origNomH:f.origNomH||f.nomH, origNomW:f.origNomW||f.nomW }], trimH, trimW, wasteArea, cuts, customActH:customH, customActW:customW, depth });
    }
  }

  // ── 3. BUTT (LINEAR): multiple stock filters butted along width ─────────────
  const byActH = {};
  for (const f of all) { const hKey = f.actH.toFixed(2); if (!byActH[hKey]) byActH[hKey]=[]; byActH[hKey].push(f); }
  ...
  const addLinearResult = (actH, filterCombo) => {
    ...
    results.push({ type: count===2?"linear-2":count===3?"linear-3":"linear-4", layout:"linear", gridRows:1, gridCols:count, stockFilters:filterCombo.map(f=>({ nomH:f.nomH, nomW:f.nomW, actH:f.actH, actW:f.actW, rotated:f.rotated, origNomH:f.origNomH||f.nomH, origNomW:f.origNomW||f.nomW })), trimH, trimW, combinedW, combinedH:actH, wasteArea, cuts, customActH:customH, customActW:customW, depth });
  };
  ...
  // ── 4. GRID: multiple stock filters in grid arrangement ─────────────────────
  const uniqueHeights = [...new Set(all.map(f => +f.actH.toFixed(2)))].sort((a,b)=>a-b);
  const uniqueWidths  = [...new Set(all.map(f => +f.actW.toFixed(2)))].sort((a,b)=>a-b);
  const findFilter = (h, w) => all.find(f => +f.actH.toFixed(2) === +h.toFixed(2) && +f.actW.toFixed(2) === +w.toFixed(2));
  ...
  const addGridResult = (rowHeights, colWidths) => {
    ...
    if (!isSafe(trimH)||!isSafe(trimW)) return;
    const gridFilters=[];
    for (let r=0;r<nRows;r++) for (let c=0;c<nCols;c++) { const f=findFilter(rowHeights[r],colWidths[c]); if (!f) return; gridFilters.push({ nomH:f.nomH, nomW:f.nomW, actH:f.actH, actW:f.actW, rotated:f.rotated, origNomH:f.origNomH||f.nomH, origNomW:f.origNomW||f.nomW, gridRow:r, gridCol:c }); }
  ...
  // ── SORT PRIORITY ──────────────────────────────────────────────────────────
  // Rule 1: Multi-yield ALWAYS before standard cuts when qty > 1
  // Rule 2: Preferred stock sizes FIRST within every tier
  // Within each tier: preferred stock → least waste → fewest stock filters → fewer cuts
  const prefScore = (r) => {
  ...
  results.sort((a, b) => {
    ...
    // Within the same tier: preferred stock is the #1 tiebreaker
    ...
    // Then: fewest stock filters needed
...
function CutDiagram({ result, compact = false, printMode = false }) {
  ...
  // Multi-yield: 1 stock → 2 custom filters, waste from middle
  if (result.layout === "multi-yield") {
    const f = result.stockFilters[0];
    const isH = result.yieldAxis === "H";
    const scale = Math.min(drawW / (isH ? f.actW : f.actW), drawH / (isH ? f.actH : f.actH));
    const rw = f.actW * scale, rh = f.actH * scale;
    const ox = (svgW - rw) / 2, oy = (svgH - rh) / 2;
    if (isH) {
      // 2x1: stacked vertically, waste strip in middle
      const ch = result.customActH * scale;
      const cw = result.customActW * scale;
      const gapH = rh - 2 * ch;
      const trimW = result.trimW > 0 ? (rw - cw) : 0;
      return (
        <svg viewBox={`0 0 ${svgW} ${svgH}`} className="w-full" style={{ maxHeight: svgH, background: svgBg }}>
          <rect x={ox} y={oy} width={rw} height={rh} fill={stockFill} stroke={stockStroke} strokeWidth={1.5} rx={2}/>
          {/* Top filter */}
          <rect x={ox} y={oy} width={cw} height={ch} fill={keepFill} fillOpacity={keepFillOp} stroke={keepStroke} strokeWidth={2} rx={2}/>
          <text x={ox+cw/2} y={oy+ch/2} textAnchor="middle" fill={keepTextFill} fontSize={compact?9:11} fontWeight="bold" fontFamily="monospace" dominantBaseline="middle">FILTER 1</text>
          {/* Middle waste/cap strip */}
          {gapH > 0 && <rect x={ox} y={oy+ch} width={cw} height={gapH} fill={wasteFill} fillOpacity={0.15} stroke={wasteStroke} strokeWidth={1} strokeDasharray="4 2"/>}
          {gapH > 0 && <text x={ox+cw/2} y={oy+ch+gapH/2} textAnchor="middle" fill={wasteTextFill} fontSize={8} fontFamily="monospace" dominantBaseline="middle">{result.trimH}" cap/waste</text>}
          {/* Bottom filter */}
          <rect x={ox} y={oy+ch+gapH} width={cw} height={ch} fill={keepFill} fillOpacity={keepFillOp} stroke={keepStroke} strokeWidth={2} rx={2}/>
          <text x={ox+cw/2} y={oy+ch+gapH+ch/2} textAnchor="middle" fill={keepTextFill} fontSize={compact?9:11} fontWeight="bold" fontFamily="monospace" dominantBaseline="middle">FILTER 2</text>
          {/* Width waste */}
          {trimW > 0 && <rect x={ox+cw} y={oy} width={rw-cw} height={rh} fill={wasteFill} fillOpacity={wasteFillOp} stroke={wasteStroke} strokeWidth={1} strokeDasharray="4 2"/>}
          {trimW > 0 && <text x={ox+cw+(rw-cw)/2} y={oy+rh/2} textAnchor="middle" fill={wasteTextFill} fontSize={8} fontFamily="monospace" dominantBaseline="middle" transform={`rotate(-90,${ox+cw+(rw-cw)/2},${oy+rh/2})`}>{result.trimW}" waste</text>}
          {/* Dimensions */}
          <line x1={ox} y1={oy-10} x2={ox+cw} y2={oy-10} stroke={dimStroke} strokeWidth={1}/>
          <text x={ox+cw/2} y={oy-15} textAnchor="middle" fill={dimFill} fontSize={fontSize} fontFamily="monospace">{result.customActW}"</text>
          <line x1={ox-10} y1={oy} x2={ox-10} y2={oy+ch} stroke={dimStroke} strokeWidth={1}/>
      ...
      // 1x2: side by side, waste strip in middle
      const ch = result.customActH * scale;
      const cw = result.customActW * scale;
      const gapW = rw - 2 * cw;
      const trimH = result.trimH > 0 ? (rh - ch) : 0;
      return (
        <svg viewBox={`0 0 ${svgW} ${svgH}`} className="w-full" style={{ maxHeight: svgH, background: svgBg }}>
          <rect x={ox} y={oy} width={rw} height={rh} fill={stockFill} stroke={stockStroke} strokeWidth={1.5} rx={2}/>
          {/* Left filter */}
          ...
          <text x={ox+cw/2} y={oy+ch/2} textAnchor="middle" fill={keepTextFill} fontSize={compact?9:11} fontWeight="bold" fontFamily="monospace" dominantBaseline="middle">FILTER 1</text>
          {/* Middle waste/cap strip */}
          {gapW > 0 && <rect x={ox+cw} y={oy} width={gapW} height={ch} fill={wasteFill} fillOpacity={0.15} stroke={wasteStroke} strokeWidth={1} strokeDasharray="4 2"/>}
          {gapW > 0 && <text x={ox+cw+gapW/2} y={oy+ch/2} textAnchor="middle" fill={wasteTextFill} fontSize={8} fontFamily="monospace" dominantBaseline="middle">{result.trimW}" cap</text>}
          {/* Right filter */}
          <rect x={ox+cw+gapW} y={oy} width={cw} height={ch} fill={keepFill} fillOpacity={keepFillOp} stroke={keepStroke} strokeWidth={2} rx={2}/>
          <text x={ox+cw+gapW+cw/2} y={oy+ch/2} textAnchor="middle" fill={keepTextFill} fontSize={compact?9:11} fontWeight="bold" fontFamily="monospace" dominantBaseline="middle">FILTER 2</text>
          {/* Height waste */}
          {trimH > 0 && <rect x={ox} y={oy+ch} width={rw} height={rh-ch} fill={wasteFill} fillOpacity={wasteFillOp} stroke={wasteStroke} strokeWidth={1} strokeDasharray="4 2"/>}
          {trimH > 0 && <text x={ox+rw/2} y={oy+ch+(rh-ch)/2} textAnchor="middle" fill={wasteTextFill} fontSize={8} fontFamily="monospace" dominantBaseline="middle">{result.trimH}" waste</text>}
          {/* Dimensions */}
          <line x1={ox} y1={oy-10} x2={ox+cw} y2={oy-10} stroke={dimStroke} strokeWidth={1}/>
          <text x={ox+cw/2} y={oy-15} textAnchor="middle" fill={dimFill} fontSize={fontSize} fontFamily="monospace">{result.customActW}"</text>
          <line x1={ox-10} y1={oy} x2={ox-10} y2={oy+ch} stroke={dimStroke} strokeWidth={1}/>
          ...
          <text x={ox+rw/2} y={oy+rh+20} textAnchor="middle" fill={stkDimFill} fontSize={fontSize-1} fontFamily="monospace">{f.actW}" stk</text>
        </svg>
      );
    }
  }
...
function calcStockSummary(cartItems) {
  ...
  for (const item of cartItems) {
    ...
    const stocksNeeded = Math.ceil(qty / yieldsPerStock);
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 280px" }}>
                    ...
                    <div style={{ padding:"10px 12px", borderRight:"1px solid #eee" }}>
                        ...
                        {r.stockFilters.map((sf, si) => {
                          ...
                          return (
                            <div key={si} style={{ fontSize:"12px", marginBottom:"3px" }}>
                              ...
                              {(() => {
                              const yieldsPerStock = r.yieldsPerStock || 1;
                              const stocksNeeded = Math.ceil(item.qty / yieldsPerStock);
                              return yieldsPerStock > 1
                                ? <span style={{ fontSize:"10px" }}> → Pull <strong>{stocksNeeded}</strong> (yields {stocksNeeded * yieldsPerStock} filters)</span>
                      ...
                      {r.layout === "multi-yield" && (
                        <div style={{ background:"#e8f5e9", border:"1px solid #4caf50", borderRadius:"3px", padding:"6px 8px", fontSize:"10px", color:"#2e7d32", fontWeight:"600", marginTop:"4px" }}>
                          ★ MULTI-YIELD: {r.yieldsPerStock} filters from 1 stock — {r.yieldAxis === "H" ? "stacked along height" : "side by side along width"} — cap/waste strip from middle ({r.yieldAxis === "H" ? `${r.trimH}"` : `${r.trimW}"`})
                        </div>
                      )}
                      {r.type !== "single" && r.layout !== "multi-yield" && (
                    ...
                    return (
                      <tr key={item.id} style={{ background:"#fff" }}>
                        ...
                        <td style={{ padding:"6px 10px", borderBottom:"1px solid #ccc" }}>
                          {r.layout==="multi-yield"?`Multi-Yield ${r.yieldsPerStock}x`:r.type==="single"?"Single Cut":r.layout==="grid"?`${r.gridRows}×${r.gridCols} Grid`:`${r.stockFilters.length}-Filter Butt`}
                        </td>
                      </tr>
                    );
                  })}
...
export default function FilterCutDB() {
  ...
  const handleSearch = useCallback(() => {
    ...
    const r = findAllCuts(h, w, depth, qty);
    setResults(r);
    setSearched(true);
    setSelectedIdx(0);
                    ...
                    <div className={`px-5 py-3 flex items-center justify-between ${selectedIdx===i?"bg-blue-100/50":i===0?"bg-emerald-100/50":"bg-slate-50"}`}>
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-slate-700">
                          Option {i+1} - {r.layout==="multi-yield"?`★ Multi-Yield (${r.yieldsPerStock}x from 1 stock)`:r.type==="single"?"Single Cut":r.layout==="grid"?`${r.gridRows}x${r.gridCols} Grid`:`${r.stockFilters.length}-Filter Butt`}
                        </span>
                      </div>
                      ...
                      <div className="px-5 py-3.5 bg-slate-50 flex items-center justify-between border-b border-slate-100">
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-xs text-slate-500">
                            {r.layout==="multi-yield"?`★ Multi-Yield (${r.yieldsPerStock}x from 1 stock)`:r.type==="single"?"Single Cut":r.layout==="grid"?`${r.gridRows}x${r.gridCols} Grid`:`${r.stockFilters.length}-Filter Butt`}
                          </span>
                        </div>
                        <div className="flex items-center gap-4">
                      ...
                      {cartItems.map((item, idx) => {
                        ...
                        return (
                          <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50">
                            ...
                            <td className="px-5 py-4 text-slate-500 text-xs">{r.layout==="multi-yield"?`★ Multi-Yield (${r.yieldsPerStock}x from 1 stock)`:r.type==="single"?"Single Cut":r.layout==="grid"?`${r.gridRows}x${r.gridCols} Grid`:`${r.stockFilters.length}-Filter Butt`}</td>
                              ...
                              {r.stockFilters.map((sf,si)=>{
                                ...
                                return (
                                  <div key={si}>
                                    {nomLabel} x {r.depth}"
                                    {yieldsPerStock > 1 
                                      ? <span className="text-emerald-600 ml-1">→ Pull {stocksNeeded} (makes {stocksNeeded * yieldsPerStock})</span>
